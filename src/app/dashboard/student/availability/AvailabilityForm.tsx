...
} from "@/server/actions/student/availability";
...
type Props = {
  ...
  /** Current saved preferences keyed by timeSlotId */
  saved: Record<string, Preference>;
};
...
const PREF_CYCLE: Preference[] = ["UNAVAILABLE", "AVAILABLE", "PREFERRED"];

const PREF_STYLE: Record<Preference, string> = {
  UNAVAILABLE:
  ...
  PREFERRED:
    "bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400 ring-1 ring-blue-300 dark:ring-blue-700",
};
...
export function AvailabilityForm({ timeSlots, saved }: Props) {
  const [state, action, pending] = useActionState(
    saveStudentAvailability,
    initial,
  );

  // Local state: map of timeSlotId → preference
  const [prefs, setPrefs] = useState<Record<string, Preference>>(() => {
    const init: Record<string, Preference> = {};
    for (const slot of timeSlots) {
      init[slot.id] = saved[slot.id] ?? "UNAVAILABLE";
    }
    return init;
  });
  ...
  function cycle(slotId: string) {
    setPrefs((prev) => {
      const current = prev[slotId] ?? "UNAVAILABLE";
  ...
  // Group unique time ranges (startTime-endTime), sorted
  ...
  const entries = Object.entries(prefs).map(([timeSlotId, preference]) => ({
    timeSlotId,
    preference,
  }));
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div className="flex items-center gap-4 text-xs">
        ...
        <span className="flex items-center gap-1.5">
          <span className="inline-block w-4 h-4 rounded bg-blue-100 dark:bg-blue-900/40 ring-1 ring-blue-300 dark:ring-blue-700" />
          Préféré
        </span>
        <span className="text-zinc-400 ml-2">Cliquer pour changer</span>
      </div>
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {timeRanges.map((range) => (
              <tr key={range} className="border-t border-zinc-200 dark:border-zinc-800">
                ...
                {activeDays.map((day) => {
                  ...
                  const pref = prefs[slot.id] ?? "UNAVAILABLE";
                  return (
                    <td key={day} className="py-2 px-1 text-center">
                      <button
                        type="button"
                        onClick={() => cycle(slot.id)}
                        className={`inline-flex items-center justify-center rounded-lg px-2 py-1.5 text-xs font-medium transition-colors min-w-[60px] ${PREF_STYLE[pref]}`}
                      >
                        {PREF_LABEL[pref]}
                      </button>
                    </td>
      ...
      {timeSlots.length === 0 && (
        <p className="text-sm text-zinc-500">
          Aucun créneau horaire configuré. Contacte ton centre.
        </p>
      )}
